...
| 10 | **Token day (live drains)** | 🟡 mostly done 2026-07-23 | Methods 4+5 drained real issues (PRs #9/#10/#12/#13; 9 engine defects found+fixed); hook fired live; kill tests passed; owner smoke closed. **Fleet real run DEFERRED by decision** (completeness not capability; integrated + per-replica PATs if ever needed). Remaining: method-1 dispatch (human's browser, low stakes) |
| SKA-011 | **The reviewer** | 🟡 2026-07-27 | Reviewer built on the SakalMaster App / `github-actions[bot]` — NO second install (ZERO-CONFIG). `reviewer.yml` + `actions/review-agent` (identity assert · idempotency per head · size guard · generated-by-marker · dangerous list · verdict discipline) · `docs/REVIEWER.md` (custody trade recorded) · `tool/test-reviewer.sh` (51 assertions). **LIVE 2026-07-28**: both gates opened (SKM-018 deployed; installation now `pull_requests: write`, `contents` still `read`). Full cycle proven on garage#129 — comment → request-changes ×2 → rework (append-only, gate re-run) → **approve** → gate correctly HELD on 3 unresolved threads. Two `allowed_bots` defects found by running it (v2.6.2/v2.6.3): in a closed agent loop every step is bot-triggered. Mint verified repo-scoped, 59-min, cannot-write-code |
| SKA-010 | **PR review loop** | 🟡 2026-07-27 | Contract **step 4b** + invariants 10–13 · `review-loop.yml` (rework · cap · anchors · nudge · override) · `actions/review-state` (one evaluator, shared by the merge gate and the loop) + `review-brief` + `review-anchors` · automerge extended to the full precondition set · `review:*` labels · `tool/test-review-loop.sh` (61 assertions) · `docs/branch-protection.md`. Found live: `files(first:300)` returned NULL, vacating the guardrail check. **Blocked on SKA-011 before `v2` moves** — `require_approval: true` with no reviewer identity stalls every agent PR |
