...
| `caller-reviewer.yml` (**built**, v2.6.0) | `.github/workflows/reviewer.yml` | the independent reviewer. `contents: read` in its permissions block is a SAFETY MECHANISM (merging needs write) — never raise it. Set `agent_authors` if this repo's agent PRs are not authored by `claude[bot]` |
